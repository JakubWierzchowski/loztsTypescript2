...
const Modal: FC<ModalAddPlayerProps> = ({ findTournaments, handleClose, isOpen, tournaments, fetchData }) => {
  ...
  return (
    <>
      <ReactModal isOpen={isOpen} onRequestClose={handleClose} ariaHideApp={false} className={styles.modalWrapper}>
        ...
        <section className={styles.wrapperForm}>
          <form className={styles.form} onSubmit={handleSubmit(handleAddPlayer, onError)} noValidate>
            <div className={`${styles.formSection}  ${styled.slideIn}`}>
              ...
              />
              {errors.zawodnicy ? <p className={styles.error}>{errors.zawodnicy?.message}</p> : null}
            </div>
            <div className={`${styles.formSection}  ${styled.slideIn}`}>
              ...
              />
              {errors.klub ? <p className={styles.error}>{errors.klub?.message}</p> : null}
            </div>
            <div className={`${styles.formSection}  ${styled.slideOut}`}>
              ...
              <select
                ...
                {...register('kategoria', {
                  ...
                  validate: (fieldValue) => {
                    return fieldValue !== 'Wybierz kategorię' || 'Wybierz kategorię!';
                  },
                })}
              >
                {findTournaments?.map(
              ...
              {errors.kategoria ? <p className={styles.error}>{errors.kategoria?.message}</p> : null}
            </div>
            <div className={`${styles.formSection}  ${styled.slideOut}`}>
            ...
            <button type="submit" className={`${styles.sendButton}  ${styled.slideIn}`}>
              Wyślij
            </button>
          </form>
        </section>
