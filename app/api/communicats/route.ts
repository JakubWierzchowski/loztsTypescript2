...
export async function POST(request: NextRequest, res: any) {
  ...
  const pathDist: string = join(process.cwd(), '/public/downloadFiles/Sezon23-24');
  ...
  try {
    ...
    if (e.code === 'ENOENT') {
      ...
      NextResponse.json({ message: 'Wystąpił błąd z utworzeniem folderu dla wybranego pliku' }, { status: 403 });
  ...
  try {
    ...
    const komunikatyData = extractPath<DownloadData[] | any>(komunikatyFilePath);

    const existingCategory = komunikatyData.find((item: any) => item.month === month);
    ...
    if (
      ...
      return NextResponse.json({ message: 'Taki komunikat juz istnieje', filename }, { status: 409 });
    ...
    await writeFile(komunikatyFilePath, JSON.stringify(komunikatyData, null, 2));

    return NextResponse.json({ message: 'Dodano komunikat:', filename }, { status: 200 });
  } catch (e) {
    console.error('Error while trying to upload a file', e);
    return NextResponse.json({ message: 'Coś poszło nie tak' }, { status: 500 });
  }
}
